# SpringRTS lobby bot configuration template (replace "SpringLobbyBot" by the actual bot name in this file and remove this line)
# SpringLobbyBot configuration module
#
...
sub new {
  ...
  if(! checkSpringLobbyBotConfig($sLog,$p_conf)) {
    $sLog->log("Unable to load main configuration parameters",1);
    return 0;
  }

  $sLog=SimpleLog->new(logFiles => [$p_conf->{""}->{logDir}."/SpringLobbyBot.log",""],
                       logLevels => [$p_conf->{""}->{logLevel},3],
                       useANSICodes => [0,1-$win],
                       useTimestamps => [1,0],
                       prefix => "[SpringLobbyBot] ");
...
sub checkSpringLobbyBotConfig {
  ...
  if(@missingParams) {
    ...
    $sLog->log("Incomplete SpringLobbyBot configuration (missing global parameters: $mParams)",1);
    return 0;
  }
